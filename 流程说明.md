...
- `app_data`：`seq + data.type + data.text`；文本上限 `APP_DATA_TEXT_MAX=512` 字节（UTF-8），
  超限发送端拒绝（`rejected/payload_too_large`），保证整帧不超过 1024 字节。

## 6. 协议

### 6.1 帧

所有 TCP 消息：**2 字节大端长度前缀 + UTF-8 JSON**，负载 1~1024 字节。空帧、超长帧、
非法 JSON、未知命令视为畸形包：丢弃并记录，同一连接连续 3 次（`malformed_max_per_conn`）
→ 主动关闭该连接。设备侧发送限速 ≤ `device_rate_limit_per_sec`（默认 50 msg/s）。

### 6.2 命令（共 5 条）

| 方向 | 命令 | 说明 |
|------|------|------|
| 设备 → PC | `device_hello` | 注册握手（必需字段 `cmd/id/fw_version/proto_ver/uptime`，`session_id` 可选） |
| PC → 设备 | `host_ack` | 确认（`ok` / `busy` / `fail`） |
| 设备 → PC | `ping` | 心跳（`seq` 单调递增） |
| PC → 设备 | `pong` | 心跳应答（回显 `seq`） |
| 双向 | `app_data` | 应用数据（`seq + data.type + data.text`） |
...
`proto_ver` 必须等于 1，否则 PC 回复 `fail/unsupported_protocol` 并关闭。

### 6.4 协议契约（protocol-contract schema 2）
...
```json
{
  "schema_version": 2, "protocol_version": 1,
...
| 跨端集成测试（sim 双进程，场景 A~H） | 8 场景 | 通过（契约一致 / 热点直连会话 / PC 退出 / 断线重连 / 第二设备拒绝 / 无目标 / app_data 边界 / 帧边界） |

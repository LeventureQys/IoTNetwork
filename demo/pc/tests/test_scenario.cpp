...
std::string make_513_scenario_file(const char *path, const std::string &text_513)
{
    std::string content =
        "{\"schema\":1,\"actions\":["
...
struct ScenarioTimingFixture {
    ...
    std::thread host_th;
    std::string catalog_dir = "run/scenario_catalog";

    void Start()
    {
        params_defaults(&params);
        params.host_tcp_port = 55954;
...
{
    EXPECT_EQ(pc_scenario_validate_backend(1), DEMO_OK);
    EXPECT_NE(pc_scenario_validate_backend(0), DEMO_OK);
}
...
TEST(Scenario, LoadValid)
{
    ...
    write_file(path,
               ...
               "\"args\":{\"name\":\"sock_send_fail\",\"argument_json\":\"{}\"}}]}");
    pc_scenario_t s;
...
TEST(Scenario, Send513RejectedWithoutTxEvent)
{
    ...
    demo_params_t params;
    params_defaults(&params);
    params.host_tcp_port = 55955;
    std::filesystem::remove_all("run/scenario_513_catalog");
    snprintf(params.sim_catalog_dir, sizeof(params.sim_catalog_dir), "%s",
             "run/scenario_513_catalog");
    ...
    make_513_scenario_file(scenario_path, text);
    pc_scenario_t scenario;
    ASSERT_EQ(pc_scenario_load(scenario_path, &scenario), DEMO_OK);

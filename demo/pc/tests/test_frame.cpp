...
TEST(FrameWrap, TooLong)
{
    ...
    EXPECT_EQ(frame_wrap(payload, (int)sizeof(payload), out, (int)sizeof(out)), DEMO_ERR_INVAL);
}
...
{
    uint8_t data[4] = {0x04, 0x01, 'x', 'x'}; /* 长度 1025 > 上限 */
    int off = 0, len = 0, consumed = 0;
...
{
    uint8_t data[PROTO_V2_HEAD_LEN] = {0, 0, 0, 5}; /* total < 12 */
    int type = 0, plen = 0, consumed = 0;
    uint64_t seq = 0;
    const uint8_t *pp = nullptr;
...
{
    uint8_t data[PROTO_V2_HEAD_LEN] = {0, 1, 0, 1}; /* total 65537 > 65536 */
    int type = 0, plen = 0, consumed = 0;
    uint64_t seq = 0;
    const uint8_t *pp = nullptr;
    EXPECT_EQ(frame_v2_parse(data, (int)sizeof(data), &type, &seq, &pp, &plen, &consumed),
